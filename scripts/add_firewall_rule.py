...
Outputs appended rules under `inet_firewall_rules` in the tfvars file.
"""
...
def main():
    ...
    # Load or initialize tfvars data
    try:
    ...
    key = "inet_firewall_rules"
    existing = data.get(key)
    if not isinstance(existing, list):
        existing = []

    # Append each rule with incremented priority and unique name
