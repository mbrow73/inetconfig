...
if len(sys.argv)!=4:
    print("Usage: update_firewall_rule.py <inputs.json> <in.tfvars> <out.tfvars>"); sys.exit(1)

data = json.load(open(sys.argv[1]))
reqid = data["reqid"].strip()
carid = data["carid"].strip()
name  = data["existing_rule_name"].strip()
src   = data["new_source_ips"].strip()
dst   = data["new_destination_ips"].strip()
pts   = data["new_ports"].strip()
proto = data["new_protocol"].strip().lower()
direc = data["new_direction"].strip().upper()
just  = data["new_justification"].strip()

# Validate required
for fld,val in [("reqid",reqid),("carid",carid),("name",name),
                ("src",src),("dst",dst),("pts",pts),
                ("proto",proto),("direc",direc),("just",just)]:
    if not val:
        print(f"❌ Missing {fld}"); sys.exit(1)
...
if name not in by_name: sys.exit(f"❌ Rule {name} not found")

old = by_name[name]
parts = name.split("-")
if len(parts)>=6:
    parts[1], parts[2] = carid, reqid
new_name = "-".join(parts) if len(parts)>=6 else name
